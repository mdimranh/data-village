...
(function ($, document, undefined) {
  ...
  var treeConfig = {
    /* properties & methods configurable*/
    menuMode: 3 /* 1 - right-click menu, 2 - icon menu, 3 - both */,
    ...
    urlAjax: "/folders" /* ajax request url */,
  };
  var change = [
    "canDrag",
    "rootLeaf",
  ...
  var treeCommands = {
    /* core set */ opn: { label: "Open", menu: false },
  ...
  var treeParm = {
    ...
    core: {
      ...
      data: function (node, callback) {
        var prm = {
          cmd: "opn",
          id: node.id === "#" ? tree.rootParent : node.id,
        };
    ...
    search: {
      ...
      search_callback: function (str, node) {
        ...
        var t = tree.search.csi
          ? node.text
          : node.text.toLowerCase(); /* haystack */
  ...
  var GetData = function (args, clb) {
    ...
    if (args.id === tree.rootParent) {
      ...
      args.ckd = tree.checkBox
        ? $(tree.Id).jstree("is_checked", args.id) === true
        : null;
    }
  ...
  var CommandItem = function (cmd, key) {
    var cmds =
      '<a href="#' + cmd + '"' + ' title="' + tree.cmdList[key].label + '"';
    if (!tree.iconPath) {
      cmds += ' class="_' + cmd + '">';
  ...
  var ContextMenu = function (coms, cms) {
    ...
    for (var cmd in coms) {
      if (coms[cmd].submenu) {
        ...
        f = false;
        coms[cmd].icon = tree.icons[cmd]
          ? tree.icons[cmd].src
          : "_" + cmd + " _cmd";
        coms[cmd]._disabled = false;
      } else {
        coms[cmd].icon = tree.icons[cmd + suffix]
          ? tree.icons[cmd + suffix].src
          : "_" + cmd + suffix + " _cmd";
        var f =
          tree.cmdList[cmd].icon &&
          tree.cmdList[cmd].icon.length > 1 &&
          tree.cmdList[cmd].icon[0] === tree.cmdList[cmd].icon[1];
  ...
  var Action = function (com, flg) {
    ...
    tree.cmdInfo.cmd =
      typeof flg === "string"
        ? flg
        : com.attr("href").substr(1).replace(suffix, "");
    ...
    var act = tree.callBacks.action
      ? tree.callBacks.action(obj)
      : tree.cmdInfo.cmd;
    if (act === "sch") {
  ...
  var ShowIcons = function (cms) {
    ...
    for (var cmd in tree.cmdList) {
      ...
      if ($.inArray(cmd, cms) + 1) {
        ...
        if (!tree.iconPath || f) {
          var s =
            f && tree.cmdList[cmd].icon[0] === tree.cmdList[cmd].icon[1]
              ? ""
              : suffix;
          c = 'a[href="#' + cmd + s + '"]';
        }
  ...
  $.fn.jsfiler = function (options) {
    ...
    var Options = function () {
      ...
      if (config.menuMode && config.menuMode !== 2) {
        //04.2017
    ...
    var Icons = function () {
      ...
      for (c in config.cmdList) {
        /* preload command images */
        if (config.cmdList[c].icon) {
          ...
          if (config.cmdList[c].icon.length > 1) {
            ...
            config.icons[c + suffix].src =
              config.iconPath + config.cmdList[c].icon[1];
          }
        }
      }
    };
      ...
      $("#" + id).on({
        ...
        "rename_node.jstree": function (e, data) {
          ...
          if (typeof title === "string") {
            ...
            Request(
              tree.cmdInfo.cmd,
              { id: data.node.id, idt: title, pid: data.node.parent },
              data
            );
          } else if (title === true) {
            /* incorrect name, retry */
        ...
        "refresh_node.jstree": function (e, data) {
          /* new selection */
          $(tree.Id).jstree("open_node", data.node);
        },
  ...
  var CheckDnd = function (obj, parent) {
    if (parent === undefined) {
      var f =
        obj.length === 1 &&
        obj[0].type !== 0; /* don't drag multiple nodes and/or root node */
    } else {
      f =
        parent.type &&
        parent.type !== 3 /* don't drop to a leaf node */ &&
        obj.parent &&
        obj.parent !== parent.id; /* don't drop to parent */
    }
  ...
  var CheckReplace = function (cmd, node, parent) {
    ...
    for (var i = 0; i < child.length; i++) {
      ...
      for (var j = 0; j < nodes.length; j++) {
        if (
          child[i].id !== nodes[j].id &&
          ((tree.nameDupl === 1 && nme === nodes[j].text) ||
            (tree.nameDupl === 0 &&
              nme.toLowerCase() === nodes[j].text.toLowerCase()))
        ) {
  ...
  var CopyMove = function (cmd, data) {
    tree.nodes[data.node.id] = [
      cmd === "mve" ? data.node.id : data.original.id,
      data.old_parent,
    ];
    tree.cmdInfo.flag--;
  ...
  var CopyCut = function (cmd, nodes) {
    for (var t in trees) {
      $(trees[t].Id)
        .find("." + pasteClass + "cpy" + "," + "." + pasteClass + "cut")
        .removeClass(pasteClass + "cpy" + " " + pasteClass + "cut");
    }
    for (var i = 0; i < nodes.length; i++) {
    ...
    if (!rsp.status) {
      ...
      if ($(tree.Id).jstree("get_type", dat.node.parent) === 2) {
        ...
        $(tree.Id).jstree(
          "set_icon",
          dat.node.parent,
          $(tree.Id).jstree(true).settings.types[1].icon
        );
      }
      ...
      for (i in arg.pid) {
        ...
        if (arg.pid[i] !== "#") {
          sid = arg.pid[i];
          if (
            $(tree.Id).jstree("get_type", sid) !== 0 &&
            !$(tree.Id).jstree("get_children_dom", sid).length
          ) {
            $(tree.Id).jstree("set_type", sid, 2);
            $(tree.Id).jstree(
              "set_icon",
              sid,
              $(tree.Id).jstree(true).settings.types[2].icon
            );
          }
        }
      }
  ...
  var ResponseRemit = function (arg, ids, dat) {
    ...
    if (
      !$.isEmptyObject(ids) &&
      $(tree.Id).jstree("get_type", "#" + dat.parent) === 2
    ) {
